...
| 字段 | 位置 | 说明 |
| --- | --- | --- |
| `schemaVersion` | `paper.json` 根级 | 试卷数据结构版本，例如 `1.1`。 |
...
| `questions[].type` | `paper.json` | 可使用 `single`、`multiple`、`indefinite` 或 `subjective`。主观题不需要 `options`，可配置 `maxLength`。 |
| `questions[].score` | `paper.json` | 该题分值，参与总分校验。 |
| `answers` | `answer-key.json` | 以题目 ID 为键，保存答案选项和解析。 |
| `answer` | `answer-key.json` | 客观题选项 key 数组；每个 key 必须存在于对应题目。 |
| `referenceAnswer` | `answer-key.json` | 主观题参考答案，提交后供考生自评。 |
...
答卷进度和成绩默认同步到 Supabase（未配置时保存在当前浏览器的 IndexedDB，不支持时回退到 localStorage）。同一账号同一试卷最多保留一个 `in_progress` 答卷；选择“重新考试”时，已完成答卷会原样保留，并创建新的答卷 ID、计时、答案和标记。若已有进行中的答卷，系统会先询问是否放弃，放弃后该记录标记为 `abandoned`，原有答案仍可在历史记录中查看。
...
客观题必须由以下三块内容组成，并以 `本题答案：` 行结束；主观题使用 `参考答案：` 行结束：
...
每道新题必须以 `【科目编号】` 开始。支持 `（单选）`、`（多选）`、`（不定项）`、`（主观题）`；单选每题 1 分、1 分钟，多选和不定项每题 2 分、2 分钟，主观题默认不计自动得分。连续答案字母会自动拆分，例如 `AB` 生成 `["A", "B"]`。科目、总分、总时长、题目 ID 和标签均由脚本自动生成。

主观题示例：

```text
【民法201】请结合案例分析并给出结论。（主观题）
